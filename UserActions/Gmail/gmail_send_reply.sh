...
# reply to the last message received: click on it to open
adb shell input tap 600 450 1>/dev/null
sleep 1.5

# click on the "Reply" button in the bottom right corner to start writing the reply
adb shell input tap 200 2500 1>/dev/null
sleep 1.5
...
if [ "$1" == 1 ]
then
	adb shell input text $(echo "Reply to message Tor:${random_string}" | sed -e 's/ /\%s/g')
else
	adb shell input text $(echo "Reply to message:${random_string}" | sed -e 's/ /\%s/g')
fi
sleep 1.5
