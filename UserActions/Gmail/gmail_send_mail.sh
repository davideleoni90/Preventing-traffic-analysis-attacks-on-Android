...
RECIPIENT="davide.leoni90@gmail.com"

# the subject of the email
SUBJECT="Seminar distributed systems"
...
# start Gmail Main Activity and wait that it's loaded
adb shell am start "com.google.android.gm/.ConversationListActivityGmail"
sleep 2

# click the button to start writing the email
adb shell input tap 1260 2380 1>/dev/null
sleep 1.5

# the focus is on the text field of the recipient: write it
adb shell input text $(echo "${RECIPIENT}" | sed -e 's/ /\%s/g')
sleep 1.5

# close the "Allow contacts suggestion" banner
adb shell input tap 1295 920 1>/dev/null
sleep 1.5

# set the focus on the subject field
adb shell input tap 300 900 1>/dev/null
sleep 1.5

# insert the subject of the email
adb shell input text $(echo "${SUBJECT}" | sed -e 's/ /\%s/g')
sleep 1.5

# set the focus on the message field
adb shell input tap 300 1100 1>/dev/null
sleep 1.5

# insert the message of the email (add a final random string)
random_string=$(getRandomString)
if [ "$1" == 1 ]
then
	adb shell input text $(echo "Message Tor:${random_string}" | sed -e 's/ /\%s/g')
else
	adb shell input text $(echo "Message:${random_string}" | sed -e 's/ /\%s/g')
fi
sleep 1.5

# click on the the "send" button
adb shell input tap 1150 200 1>/dev/null
sleep 1.5
