...
INTERFACE="ra0"
DEVICE_IP="146.193.41.253"
CAPTURE_FILTER="host ${DEVICE_IP}"
DISPLAY_FILTER="not arp and not bjnp and not dns and not ntp and not(ip.src==216.58.192.0/19 or ip.dst==216.58.192.0/19) and not tcp.analysis.retransmission and not tcp.analysis.fast_retransmission and ssl"
TIMEOUT=10
...
# if the only parameter passed to the script is 0 the default network is being used, while if it's 1 the Tor network is being used: in this case add a suffix to the the .pcap and .csv files produced in order to distinguish the between the different configurations
...
then
	...
	adb shell input tap 600 1760 1>/dev/null
