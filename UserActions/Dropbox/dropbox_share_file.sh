...
# the email of the individual with whom the file is to be shared
RECIPIENT="leoni.1614811@studenti.uniroma1.it"
...
# click on the search button to find the file to be shared
adb shell input tap 1000 200 1>/dev/null
sleep 1.5

# insert the name of the file to be shared
...
adb shell input tap 600 1400 1>/dev/null
sleep 5

# refuse to grant access to contacts
adb shell input tap 755 1555 1>/dev/null
sleep 1.5

# insert the name of the "recipient"
adb shell input text $(echo "${RECIPIET}" | sed -e 's/ /\%s/g')
sleep 1.5
