...
CONTENT="This is an example of text file"
...
# select "Create new text file" in the context menu
adb shell input tap 600 2000 1>/dev/null
sleep 1.5
...
# click on the icon "Save File" in the top right corner
adb shell input tap 1350 200 1>/dev/null
sleep 1.5

# insert the filename (the focus is already in the text input)
if [ "$1" == 0 ]
then
	adb shell input text $(echo "${NAME}" | sed -e 's/ /\%s/g')
else
	adb shell input text $(echo "${NAME}_tor" | sed -e 's/ /\%s/g')
fi
sleep 1.5 
...
adb shell input tap 1165 1555 1>/dev/null
