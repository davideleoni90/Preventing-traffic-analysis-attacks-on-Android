...
MESSAGE="Hi! This a test for a project"
...
OUTPUT_PCAP="Traces/Facebook/${FILENAME%.*}"
...
# focus on the "Search" text field (suppress warning messages)
adb shell input tap 250 200 1>/dev/null
sleep 1.5

# write the name of the facebook friends to whom we want to write a post
adb shell input text $(echo "${FRIEND}" | sed -e 's/ /\%s/g')
sleep 1.5

# select the first suggestion for the term to search, i.e. the contact name
adb shell input tap 200 350 1>/dev/null
sleep 1.5

# select the first search result
adb shell input tap 150 750 1>/dev/null
sleep 1.5

# scroll down till the "Write Post" label
adb shell input swipe 125 2000 125 100 1>/dev/null
sleep 3

# click on "Write Post"
adb shell input tap 230 1050 1>/dev/null
sleep 1.5

# write the text to be published in the friend's wall
if [ "$1" == 1 ]
then
	adb shell input text $(echo "${MESSAGE} Tor:" | sed -e 's/ /\%s/g')
else
...
# click on "Post" to publish the update
adb shell input tap 1300 185 1>/dev/null
sleep 1.5
