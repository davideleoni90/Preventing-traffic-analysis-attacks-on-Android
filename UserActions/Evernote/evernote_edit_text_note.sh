...
# click on the most recent note in order to modify it 
adb shell input tap 400 600 1>/dev/null
sleep 3

# click on the bottom right button "Edit note" 
adb shell input tap 1250 2350 1>/dev/null
sleep 1.5

# the focus is on the content of the note: append some new content
if [ "$1" == 0 ]
then
	adb shell input text $(echo "${NEW_CONTENT}:" | sed -e 's/ /\%s/g')
else
	adb shell input text $(echo "${NEW_CONTENT} Tor:" | sed -e 's/ /\%s/g')
fi
sleep 1.5
