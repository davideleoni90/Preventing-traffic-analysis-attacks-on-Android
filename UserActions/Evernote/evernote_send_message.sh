...
# the textual message to be sent
MESSAGE="This is just a trial for a seminar project"
...
# start Evernote Main Activity and wait that it's loaded
adb shell am start "com.evernote/.ui.HomeActivity"
sleep 3

# open the lateral left menu 
adb shell input tap 100 200 1>/dev/null
sleep 1.5

# select the "Work Chat" 
adb shell input tap 600 1550 1>/dev/null
sleep 1.5

# select the top contact shown
adb shell input tap 600 1000 1>/dev/null
sleep 1.5

# the focus is on the text field for the message to be sent: insert the message
if [ "$1" == 0 ]
then
...
# send the message
adb shell input tap 1250 2450 1>/dev/null
sleep 2
