...
# click on the button in the top right corner of the screen to send a new message
adb shell input tap 1250 400 1>/dev/null
sleep 1.5
...
# write the name of the recepient in the search box (focus is already there)
adb shell input text $(echo "${RECIPIENT}" | sed -e 's/ /\%s/g')
sleep 1.5

# select the first result
adb shell input tap 200 650 1>/dev/null
sleep 1.5

# click on NEXT
adb shell input tap 1320 200 1>/dev/null
sleep 1.5

# focus on the text box
adb shell input tap 400 2450 1>/dev/null
sleep 1.5
...
if [ "$1" == 0 ]
then
adb shell input text $(echo "Ciao! Seminar project trial:${random_string}" | sed -e 's/ /\%s/g')
else
adb shell input text $(echo "Ciao! Seminar project trial tor:${random_string}" | sed -e 's/ /\%s/g')
fi
sleep 1.5

# send the message
adb shell input tap 1320 2450 1>/dev/null
sleep 1.5
