...
# time at the moment when the capture starts (in seconds): we will consider only packet whose timestamp is greater than this, in order to be sure to analyse only the traffic generated by the user action.
